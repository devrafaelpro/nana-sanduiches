// Elementos exibidos APENAS em deploys de demonstração.
// Liga com as variáveis de ambiente:
//   NEXT_PUBLIC_DEMO_MODE=1
//   NEXT_PUBLIC_DEMO_WHATSAPP=5511999999999  (WhatsApp de quem vende o sistema)
// Nos projetos pagos dos clientes essas variáveis não existem e nada é renderizado.
export default function DemoBanner() {
  ...
  const msg = encodeURIComponent(
    "Olá! Vi a demonstração e quero ativar esse cardápio digital pro meu negócio 🍔"
  );
  const link = `https://wa.me/${phone}?text=${msg}`;

  return (
    <>
      {/* Faixa fixa no topo — deixa claro que é uma demonstração */}
      <a
        href={link}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed inset-x-0 top-0 z-50 flex items-center justify-center gap-2 bg-amber-500 px-4 py-1.5 text-center text-xs font-bold text-black"
      >
        <span className="animate-pulse">🔓</span>
        DEMONSTRAÇÃO — toque aqui pra ativar o seu cardápio
      </a>
